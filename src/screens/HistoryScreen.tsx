...
export function HistoryScreen({ workouts, completedWorkouts, accent = ACCENT_DEFAULT, onOpen }: HistoryScreenProps) {
  ...
  return (
    <div className="pb-10">
      {/* Stats strip */}
      <div className="relative text-white" style={{ background: 'var(--hero)', padding: '16px 20px 20px' }}>
        ...
        <div className="grid grid-cols-3 gap-3 mt-3.5">
          <div>
            <div className="font-mono text-[10px]" style={{ letterSpacing: 1, color: 'var(--on-hero-2)' }}>STREAK</div>
            <div className="text-[28px] font-[800] mt-0.5" style={{ letterSpacing: -1 }}>
              {streak}<span className="text-[12px] ml-1" style={{ color: 'var(--on-hero-2)' }}>days</span>
            </div>
          </div>
          <div>
            <div className="font-mono text-[10px]" style={{ letterSpacing: 1, color: 'var(--on-hero-2)' }}>SESSIONS</div>
            <div className="text-[28px] font-[800] mt-0.5" style={{ letterSpacing: -1 }}>{done}</div>
          </div>
          <div>
            <div className="font-mono text-[10px]" style={{ letterSpacing: 1, color: 'var(--on-hero-2)' }}>TIME</div>
            <div className="text-[28px] font-[800] mt-0.5" style={{ letterSpacing: -1 }}>
              {Math.round(totalMins / 60)}<span className="text-[12px] ml-1" style={{ color: 'var(--on-hero-2)' }}>hr</span>
            </div>
          </div>
        </div>
          ...
          {pastWorkouts.slice(0, 7).reverse().map((w, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="font-mono text-[8px] font-bold" style={{ letterSpacing: 0.5, color: 'var(--on-hero-3)' }}>
      ...
      <div className="px-4 pt-4">
        {weeks.map(group => (
          <div key={group.label} className="mb-4">
            <div className="font-mono text-[11px] font-bold px-1 pb-2.5 pt-1" style={{ letterSpacing: 2, color: 'var(--text-2)' }}>
              {group.label.toUpperCase()}
            </div>
            <div className="overflow-hidden" style={{ background: 'var(--surface)', borderRadius: 20, border: '1px solid var(--hairline)' }}>
              {group.items.map((w, i) => {
                const isCompleted = completedIds.has(w.workout_id)
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '14px 16px',
                      borderBottom: i < group.items.length - 1 ? '1px solid var(--hairline)' : 'none',
                    }}
                  >
                    {/* date block */}
                    <div className="w-[42px] text-center flex-shrink-0">
                      <div className="font-mono text-[9px] font-bold" style={{ letterSpacing: 0.8, color: 'var(--text-3)' }}>
                        {w.day_of_week.toUpperCase()}
                      </div>
                      <div className="text-[18px] font-[800] leading-none mt-0.5" style={{ letterSpacing: -0.5, color: 'var(--text-1)' }}>
                        {dayNum}
                      </div>
                    </div>
                    {/* name */}
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        ...
                        <span className="font-mono text-[10px]" style={{ letterSpacing: 0.5, color: 'var(--text-3)' }}>
                    ...
                    {isCompleted ? (
                      <div className="w-[26px] h-[26px] rounded-full flex items-center justify-center flex-shrink-0" style={{ background: accent }}>
                        <svg width="12" height="10" viewBox="0 0 12 10">
                          <path d="M1 5L4.5 8.5L11 1.5" stroke="var(--accent-on)" strokeWidth="2.2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </div>
                    ) : (
                      <div className="font-mono text-[10px] font-bold" style={{ color: 'var(--text-3)', letterSpacing: 0.5 }}>
